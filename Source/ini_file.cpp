...
std::unique_ptr<std::vector<std::shared_ptr<ini_section>>>
ini_file::get_sections() const {
  auto v = std::make_unique<std::vector<std::shared_ptr<ini_section>>>();
  std::for_each(std::begin(smap), std::end(smap),
...
bool ini_file::add_section(std::string const& section_name,
                           std::string* parent_name = nullptr) {
  auto const& parent = parent_name != nullptr ? get_section(*parent_name)
                                              : std::weak_ptr<ini_section>{};
  std::shared_ptr<ini_section> section = std::make_shared<ini_section>(
      this->weak_from_this(), parent, section_name);
